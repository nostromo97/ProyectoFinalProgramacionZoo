...
public class Primate extends Animal {
	/**
	 * Variable de tipo boolean que indica el género del primate, masculino o
	 * femenino como los valores true o false.
	 */
	 ...
	 * @param genero          Variable de tipo boolean que indica el género del
	 *                        primate, masculino o femenino como los valores true o
	 *                        false.
	...
	public Primate(short id, LocalDate fechaNacimiento, String nombre, TipoRaza raza, String cuidados,
			...
			FechaFormatoException, CampoVacioException, MotivoVacioException {
	 ...
	 *               animal.
	 * @param genero Variable de tipo boolean que indica el género del primate,
	 *               masculino o femenino como los valores true o false.
	 * @throws NombreVacioException    Excepción que se lanza cuando el nombre está
	 *                                 vacío.
	 * @throws NombreInvalidoException Excepción que se lanza cuando el nombre no es
	 *                                 válido.
	...
	/**
	 * Constructor de la clase Primate que extiende de Animal, utilizado para dar de
	 * alta a un Primate. Hace conexión con la base de datos e inserta un primate.
	 * 
	 * @param nombre          Cadena de caracteres que contiene el nombre del
	 *                        animal.
	 * @param fechaNacimiento Variable de tipo LocalDate que contiene la fecha de
	...
	public Primate(String nombre, LocalDate fechaNacimiento, MotivoAlta motivoAlta, LocalDate fechaAlta, boolean genero,
			...
			CampoVacioException, MotivoVacioException, GeneroVacioException, DescripcionVaciaException {
		...
		if (query.executeUpdate("insert into altaPrimate values(null,'" + nombre + "','" + fechaNacimiento + "','"
				+ motivoAlta + "','" + fechaAlta + "'," + genero + ",'" + cuidados + "')") > 0) {
			this.genero = genero;
		} else {
	 ...
	 *                                 selecciona ningún motivo.
	 */
	public Primate(short id, String nombre, LocalDate fechaNacimiento, MotivoBaja motivoBaja, LocalDate fechaBaja,
			boolean genero, String cuidados) throws NombreVacioException, NombreInvalidoException,
			FechaFormatoException, SQLException, MotivoVacioException, IdVacioException, IdInvalidoException {
		super(id, nombre, fechaNacimiento, motivoBaja, fechaBaja, cuidados);
		Scanner sc = new Scanner(System.in);
		Statement query = UtilsDB.conectarBD();

		if (query.executeUpdate("insert into bajaPrimate values(null,'" + nombre + "','" + fechaNacimiento + "','"
				+ motivoBaja + "','" + fechaBaja + "'," + genero + ",'" + cuidados + "')") > 0) // insert en baja
		{
			this.genero = genero;
